return {
    descriptions = {
        Back = {
            b_gems_crystal = {
                name = "Crystal Deck",
                text = {
                    "All playing cards start",
                    "with an {C:empty}Empty{} Gemstone Slot"
                }
            }
        },
        Blind = {
            bl_gems_rock = {
                name = "The Rock",
                text = {
                    "All cards with an occupied",
                    "Gemstone Slot are debuffed"
                }
            },
            bl_gems_hammer = {
                name = "The Hammer",
                text = {
                    "Lose $3 for every card played",
                    "with an occupied Gemstone Slot"
                }
            }
        },
        Edition = {},
        Gemstone = {
            c_gems_ruby = {
                name = "Ruby",
                text = {
                    "Attach a {C:ruby}Ruby",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_pearl = {
                name = "Pearl",
                text = {
                    "Attach a {C:pearl}Pearl",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_topaz = {
                name = "Topaz",
                text = {
                    "Attach a {C:topaz}Topaz",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_amber = {
                name = "Amber",
                text = {
                    "Attach an {C:amber}Amber",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_opal = {
                name = "Opal",
                text = {
                    "Attach an {C:opal}Opal",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_diamond = {
                name = "Diamond",
                text = {
                    "Attach a {C:diamond}Diamond",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_amethyst = {
                name = "Amethyst",
                text = {
                    "Attach an {C:amethyst}Amethyst",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_aquamarine = {
                name = "Aquamarine",
                text = {
                    "Attach an {C:aquamarine}Aquamarine",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_jade = {
                name = "Jade",
                text = {
                    "Attach a {C:jade}Jade",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_quartz = {
                name = "Quartz",
                text = {
                    "Attach a {C:quartz}Quartz",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_emerald = {
                name = "Emerald",
                text = {
                    "Attach an {C:emerald}Emerald",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_turquoise = {
                name = "Turquoise",
                text = {
                    "Attach a {C:turquoise}Turquoise",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_epidote = {
                name = "Epidote",
                text = {
                    "Attach an {C:epidote}Epidote",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_adamite = {
                name = "Adamite",
                text = {
                    "Attach an {C:adamite}Adamite",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_obsidian = {
                name = "Obsidian",
                text = {
                    "Attach a {C:obsidian}Obsidian",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_sapphire = {
                name = "Sapphire",
                text = {
                    "Attach a {C:sapphire}Sapphire",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_aventurine = {
                name = "Aventurine",
                text = {
                    "Attach an {C:aventurine}Aventurine",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_timecrystal = {
                name = "Time Crystal",
                text = {
                    "Attach a {C:timecrystal}Time Crystal",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected {C:attention}card{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
            c_gems_citrine = {
                name = "Citrine",
                text = {
                    "Attach a {C:citrine}Citrine",
                    "{C:hearts}Gemstone{} to {C:green}#1#{} selected",
                    "{C:attention}card{} or {C:attention}Joker{}",
                    "{C:inactive}(Must have a Gemstone Slot){}"
                }
            },
        },
        Joker = {
            j_gems_drill_miner = {
                name = "Drill Miner",
                text = {
                    "Create a {C:hearts}Gemstone{} card",
                    "when {C:attention}Blind{} is selected",
                    "{C:inactive}(Must have room)",
                }
            },
            j_gems_gem_gauntlet = {
                name = "Gemstone Gauntlet",
                text = {
                    "Gives {X:mult,C:white} X#1# {} Mult",
                    "for each card with ",
                    "an {C:attention}occupied Gemstone Slot{}",   
                    "in your {C:attention}full deck",
                    "{C:inactive}(Currently {X:mult,C:white} X#2# {C:inactive} Mult)",
                },
            },
            j_gems_blacksmith = {
                name = "Blacksmith",
                text = {
                    "If the {C:attention}first card{} in the",
                    "{C:attention}first hand{} has no Gemstone Slot,",
                    "create an {C:empty}Empty{} Gemstone Slot"
                }
            },
            j_gems_domini = {
                name = "Domini",
                text = {
                    "Retrigger all scoring cards",
                    "for every {C:attention}#1#{} unique {C:hearts}Gemstones",
                    "applied to {C:attention}cards{} and {C:attention}Jokers",
                    "{C:inactive}(Currently {C:attention}#2# {C:inactive}times)"
                }
            }
        },
        Planet = {},
        Sleeve = {
            sleeve_gems_crystal_sleeve = {
                name = "Crystal Sleeve",
                text = {
                    "All playing cards in your starting",
                    "deck have an {C:empty}Empty{} Gemstone Slot"
                }
            },
            sleeve_gems_crystal_sleeve_alt = {
                name = "Crystal Sleeve",
                text = {
                    "All playing cards that appear",
                    "during the run will",
                    "have an {C:empty}Empty{} Gemstone Slot"
                }
            }
        },
        Spectral = {
            c_gems_shine = {
                name = "Shine",
                text = {
                    "Select up to {C:green}#1#{} {C:attention}cards{} to attach",
                    "a random compatable {C:hearts}Gemstone{}",
                    "{C:inactive}(Gemstone Slot not required)"
                }
            },
            c_gems_greed = {
                name = "Greed",
                text = {
                    "Select {C:green}#1#{} {C:attention}Joker{} to attach",
                    "a random compatable {C:hearts}Gemstone{}",
                    "{C:inactive}(Gemstone Slot not required)"
                }
            }
        },
        Stake = {},
        Tag = {
            tag_gems_gemstone = {
                name = "Gemstone Tag",
                text = {
                    "Gives a free",
                    "{C:hearts}Mega Gemstone Pack"
                }
            },
            tag_gems_infused = {
                name = "Infused Tag",
                text = {
                    "Next Joker in shop",
                    "is free and has a",
                    "{V:1}#1#{} Gemstone Slot applied"
                }
            }
        },
        Tarot = {
            c_gems_infusion = {
                name = "The Infusion",
                text = {
                    "Create an {C:empty}Empty{}",
                    "Gemstone Slot to {C:green}2{}",
                    "selected {C:attention}cards{} or {C:attention}Jokers{}",
                    "{C:inactive}(Can override occupied slots)"
                }
            },
            c_gems_excavator = {
                name = "The Excavator",
                text = {
                    "Creates the last",
                    "{C:hearts}Gemstone{} card used",
                    "during this run"
                },
            },
            c_gems_polish = {
                name = "The Polish",
                text = {
                    "Remove the stone from",
                    "a {C:attention}Stone{} card and",
                    "create {C:green}#1#{} random",
                    "{C:hearts}Gemstone{} card"
                },
            }
        },
        Voucher = {
            v_gems_gemstone_merchant = {
                name = "Gemstone Merchant",
                text = {
                    "{C:hearts}Gemstone{} cards appear",
                    "{C:attention}#1#X{} more frequently",
                    "in the shop",
                }
            },
            v_gems_gemstone_tycoon = {
                name = "Gemstone Tycoon",
                text = {
                    "{C:hearts}Gemstone{} cards appear",
                    "{C:attention}#1#X{} more frequently",
                    "in the shop",
                }
            }
        },
        Other = {
            gemslot_empty = {
                name = "Empty",
                text = {
                    "Can be used to",
                    "hold Gemstones"
                }
            },
            gemslot_ruby = {
                name = "Ruby",
                text = {
                    "{X:mult,C:white}X#1#{} Mult",
                    "when scored"
                },
            },
            gemslot_pearl = {
                name = "Pearl",
                text = {
                    "This card is",
                    "always scored"
                },
            },
            gemslot_topaz = {
                name = "Topaz",
                text = {
                    "Earn {C:money}$#1#{} when this card",
                    "is played and scores",
                    "For Jokers, earn {C:money}$#1#{} at",
                    "end of round"
                },
            },
            gemslot_amber = {
                name = "Amber",
                text = {
                    "{C:green}#1# in #2#{} chance",
                    "to level up played hand",
                    "when card is scored"
                },
            },
            gemslot_opal = {
                name = "Opal",
                text = {
                    "This card is",
                    "considered a",
                    "face card"
                },
            },
            gemslot_diamond = {
                name = "Diamond",
                text = {
                    "Retrigger",
                    "this card",
                    "{C:attention}#1#{} time"
                },
            },
            gemslot_amethyst = {
                name = "Amethyst",
                text = {
                    "{X:mult,C:white} X#1# {} Mult",
                    "while this card",
                    "stays in hand",
                },
            },
            gemslot_aquamarine = {
                name = "Aquamarine",
                text = {
                    "{X:chips,C:white}X#1#{} Chips",
                    "when scored"
                },
            },
            gemslot_jade = {
                name = "Jade",
                text = {
                    "This card's rank",
                    "is upgraded by",
                    "{C:attention}1{} when played"
                },
            },
            gemslot_quartz = {
                name = "Quartz",
                text = {
                    "This card",
                    "permanently gains",
                    "+#1# {C:chips}Chips{}",
                    "when scored"
                },
            },
            gemslot_emerald = {
                name = "Emerald",
                text = {
                    "When {C:attention}discarded{}, {C:green}#1# in #2#{} chance",
                    "to add {C:dark_edition}Foil{}, {C:dark_edition}Holographic{}, or",
                    "{C:dark_edition}Polychrome{} edition to",
                    "a random card in {C:attention}hand"
                },
            },
            gemslot_turquoise = {
                name = "Turquoise",
                text = {
                    "If scored and first in hand,",
                    "{C:attention}destroy{} this card and create",
                    "{C:attention}#1#{} {C:dark_edition}Negative{} {C:planet}Planet{} for",
                    "played poker hand"
                },
            },
            gemslot_epidote = {
                name = "Epidote",
                text = {
                    "Increases the values",
                    "of this Joker by {C:attention}#1#% ",
                    "after {C:attention}Blind{} is defeated"
                },
            },
            gemslot_adamite = {
                name = "Adamite",
                text = {
                    "{C:green}#1# in #2#{} chance",
                    "to retrigger this",
                    "Joker {C:attention}#3#{} time"
                },
            },
            gemslot_obsidian = {
                name = "Obsidian",
                text = {
                    "When this Joker is {C:attention}sold or destroyed,",
                    "destroy this Gemstone Slot and create a",
                    "random Joker of the same rarity"
                }
            },
            gemslot_sapphire = {
                name = "Sapphire",
                text = {
                    "This card cannot be debuffed,",
                    "if this card is a {C:attention}playing card",
                    "then it acts as all suits"
                }
            },
            gemslot_aventurine = {
                name = "Aventurine",
                text = {
                    "{X:mult,C:white}X#1#{} Mult when played",
                    "and {C:money}$#2#{} when discarded",
                    "This card is always flipped"
                }
            },
            gemslot_timecrystal = {
                name = "Time Crystal",
                text = {
                    "This card returns",
                    "to {C:attention}your deck",
                    "when played"
                },
            },
            gemslot_citrine = {
                name = "Citrine",
                text = {
                    "Increase all",
                    "{C:green}probabilities{} on",
                    "this card by {C:green}1"
                }
            },
            p_gems_gemstone_normal_1 = {
                name = "Gemstone Pack",
                text = {
                    "Choose {C:attention}#1#{} of up to",
                    "{C:attention}#2#{C:hearts} Gemstone{} cards",
                    "or {C:tarot}The Infusion{} card"
                }
            },
            p_gems_gemstone_normal_2 = {
                name = "Gemstone Pack",
                text = {
                    "Choose {C:attention}#1#{} of up to",
                    "{C:attention}#2#{C:hearts} Gemstone{} cards",
                    "or {C:tarot}The Infusion{} card"
                }
            },
            p_gems_gemstone_jumbo_1 = {
                name = "Jumbo Gemstone Pack",
                text = {
                    "Choose {C:attention}#1#{} of up to",
                    "{C:attention}#2#{C:hearts} Gemstone{} cards",
                    "or {C:tarot}The Infusion{} card"
                }
            },
            p_gems_gemstone_mega_1 = {
                name = "Mega Gemstone Pack",
                text = {
                    "Choose {C:attention}#1#{} of up to",
                    "{C:attention}#2#{C:hearts} Gemstone{} cards",
                    "or {C:tarot}The Infusion{} card"
                }
            },
        },
    },
    misc = {
        achievement_names = {},
        achievement_descriptions = {},
        challenge_names = {
            c_gems_amber_gamble = "Amber Gamble",
            c_gems_only_flushes = "Only Flushes",
        },
        dictionary = {
            b_pull = "PULL",
            k_applied_ex = "Applied!",
            k_plus_gemstone = "+1 Gemstone",
            k_gems_gemstone_pack = "Gemstone Pack"
        },
        labels = {
            gemslot = "Gemstone Slot"
        },
        v_dictionary = {
            a_xchips = {"X#1# Chips"},
            a_powmult = {"^#1# Mult"},
            a_powchips = {"^#1# Chips"},
            a_powmultchips = {"^#1# Mult+Chips"},
            a_round = {"+#1# Round"},
            a_xchips_minus = {"-X#1# Chips"},
            a_powmult_minus = {"-^#1# Mult"},
            a_powchips_minus = {"-^#1# Chips"},
            a_powmultchips_minus = {"-^#1# Mult+Chips"},
            a_round_minus = {"-#1# Round"},
        },
        v_text = {
            ch_c_gems_no_planets = {"{C:planet}Planet{} cards never spawn"},
            ch_c_gems_lock_slot_sapphire = {"All playing cards are locked to {C:sapphire}Sapphire{} Gemstone Slots"},
        }
    }
}